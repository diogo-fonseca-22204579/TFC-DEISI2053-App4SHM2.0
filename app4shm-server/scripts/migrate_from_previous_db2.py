...
def run():
    ...
    # Baloico
    s1 = Structure.objects.filter(name="Baloico")[0]

    # passagemRigidaCG
    s2 = Structure.objects.filter(name='passagemRigidaCG')[0]

    # Itacaiunas Nova E5
    s3 = Structure.objects.filter(name='Itacaiunas Nova E5')[0]

    # Itacaiunas Nova E2
    s4 = Structure.objects.filter(name='Itacaiunas Nova E2')[0]

    # Itacaiunas Velha E2
    s5 = Structure.objects.filter(name='Itacaiunas Velha E2')[0]

    # Itacaiunas velha E5
    s6 = Structure.objects.filter(name='Itacaiunas velha E5')[0]

    # passagemFlexivelCG
    s7 = Structure.objects.filter(name='passagemFlexivelCG')[0]

    # PassagemFlexivelDano
    s8 = Structure.objects.filter(name='PassagemFlexivelDano')[0]

    with open('./scripts/app4shm_db_2.csv') as csv_file:
